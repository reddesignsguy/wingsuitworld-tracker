...
const {
  getImageUrl,
} = require("./apis/robloxUserMediaApi/robloxUserMedia.api.accessors"); // TODO Change to import statement
const {
  getRank,
  getMaps,
  getTopScore,
  getTotalScore,
  getProfileCode,
} = require("./apis/datastoreApi/datastore.api.accessors");
const {
  getUserId,
  getDisplayName,
} = require("./apis/robloxUserApi/robloxUser.api.accessors");
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      reject(
        response(
          404,
          `Can't claim a profile for a user that doesn't exist. DB message: ${err.message}`
        )
      );
      return;
    }
    ...
    if (existingUserData.message.playerName != null) {
      reject(
        response(
          409,
          "User has already claimed a profile. User should unclaim user`s profile first."
        )
      );
      return;
    }
    ...
    try {
      ...
      reject(
        response(
          404,
          `Player does not exist in the datastore. Error: ${JSON.stringify(
            err.message
          )}`
        )
      );
    ...
    var datastoreApiResponse;
    try {
      datastoreApiResponse = await datastoreApi.fetch(playerId);
    } catch (err) {
      reject(response(404, err.message));
      return;
    }
    ...
    if (getProfileCode(datastoreApiResponse) != profileCode) {
      reject(
        response(
          401,
          "Incorrect profile code given. User should log into the game to find the user`s correct profile code"
        )
      );
      return;
    }
    ...
    try {
      ...
      resolve(
        response(
          201,
          `${userId} has claimed ${playerName}'s profile. Result: ${JSON.stringify(
            dbResult.message
          )}`
        )
      );
      ...
      if (err.status_code == 409) {
        reject(
          response(
            err.status_code,
            `Another user has already claimed ${playerName}'s profile! DB Error: ${err.message}`
          )
        );
        return;
      }
      ...
      reject(
        response(
          err.status_code,
          `Attempted to claim profile but a database error occurred: ${err.message}`
        )
      );
    }
  });
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      resolve(
        response(
          201,
          `${userId} has unclaimed a profile. Result: ${JSON.stringify(
            dbResult.message
          )}`
        )
      );
      ...
      reject(
        response(
          err.status_code,
          `Attempted to unclaim profile but a database error occurred: ${err.message}`
        )
      );
    }
  });
